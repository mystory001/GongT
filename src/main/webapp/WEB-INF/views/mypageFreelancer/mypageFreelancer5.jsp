				...
				<fieldset
					...
					<table class="table">
						<tr style="background-color: transparent !important;">
							<td
								style="font-size: 20px !important; color: black; text-align: center !important;">보낸사람</td>
							<td
								style="font-size: 20px !important; color: black; text-align: center !important;">받은내용</td>
							<td
								style="font-size: 20px !important; color: black; text-align: center !important;">시간</td>
							<td
						...
						<c:forEach var="chattingDTO" items="${chattingDTOListF}">
							<c:if test="${chattingDTO.cid eq chattingDTO.id}">
								...
								<c:choose>
									<c:when
										test="${lastChat == '' || chattingDTO.c_time > lastChat.c_time}">
										<!-- 최신 메시지만 출력 -->
										<tr>
										...
										<c:set var="lastChat" value="${chattingDTO}" />
										<!-- 최신 메시지로 lastChat 업데이트 -->
									</c:when>
								</c:choose>
							</c:if>
						</c:forEach>
