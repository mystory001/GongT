	...
	<div id="page-container" class="main-admin">
		<nav
			...
			<a href="${pageContext.request.contextPath}/main/main"
				class="nav-links d-block" style="color: white"> <i
				class="fa fa-home pr-2" style="color: white"></i>메인 페이지
			</a>
		...
		<div class="main-body-content w-100 ets-pt">
			<div class="table-responsive bg-light">
				<table class="table">
					...
					<%-- 
					일자 : 241207 
					수정내역 : AdminController에서 List객체의 변수명 변경(ProjectDTOList -> projectDTOList)
					--%>
					<c:forEach var="projectDTO" items="${projectDTOList}">
						<div>
							<tr>
								<th>${projectDTO.id}</th>
								<th
									onclick="location.href='${pageContext.request.contextPath}/project/project?p_num=${projectDTO.p_num }'">${ProjectDTO.p_title}</th>
								<th
									onclick="location.href='${pageContext.request.contextPath}/project/project?p_num=${projectDTO.p_num }'">${ProjectDTO.p_content}</th>
								<th>${projectDTO.p_writedate}</th>
								<th>${projectDTO.p_deadline}</th>
								<th>${projectDTO.p_member}</th>
								<th><input type="button" value="글삭제"
									style="font-size: 8px;"
									onclick="location.href='${pageContext.request.contextPath}/adminPage/adminDeletePro?p_num=${projectDTO.p_num}'"></th>
							</tr>
